...
public abstract class UsbSpiDevice implements UsbSpiInterface
{
    ...
    protected static final int USB_TIMEOUT = 10000;  //OSM20250123 : 시리얼통신 연결 끊김 증상의 이유로 USB Timout 시간 변경 (5000 -> 10000)
    ...
    public static UsbSpiDevice createUsbSerialDevice(UsbDevice device, UsbDeviceConnection connection, int iface)
    {
        ...
        if(vid == 1027) { //20200413 : FTDI ATTACH시
...
//            return null;
    ...
    protected class WriteThread extends AbstractWorkerThread
    {
        ...
        public void doRun()
        {
            synchronized (serialBuffer) {   //OSM20250123 : 시리얼 통신 연결 끊김 증상의 이유로 Write/Read Buffer 동기화 처리
